...
- **project stack constraints** — every design must respect:
  - **Vertical slice**: each module (`session/`, `iteration/`, `agent/`, `state/`, `convergence/`, `markdown/`) owns its own handler, service, repository, and domain model — no shared horizontal layers
  - **No cross-module DB access**: modules interact via service calls or DTOs, never by sharing repositories
  - **Backend**: Go 1.26, modular monolith, sqlc/pgx (no heavy ORM)
  - **Frontend**: SvelteKit (latest stable), TypeScript, TailwindCSS, Svelte stores for state
  - **Agent protocol**: A2A via a2a-go; agents are separate services that accept `{task_id, role, state}` and return structured JSON
  - **LLM**: always program against the `LLMProvider` interface — never call Copilot or Claude directly
  - **Determinism**: same input + same config = identical output (no stochastic side effects in business logic)
...
- [ ] Stack constraints respected (vertical slice, Go 1.26, SvelteKit, A2A contract)
