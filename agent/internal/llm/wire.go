...
// support a two-message layout. When Tiered is set, blocks/messages are flattened.
...
func WireMessages(req LLMRequest) []PromptBlock {
	if req.Tiered == nil {
		sys, user := req.SystemPrompt, req.UserMessage
		out := make([]PromptBlock, 0, 2)
		if sys != "" {
			out = append(out, PromptBlock{Role: "system", Content: sys, CachePolicy: CacheEphemeral})
		}
		if user != "" {
			out = append(out, PromptBlock{Role: "user", Content: user, CachePolicy: CacheNone})
		}
		return out
	}
